        ...
        while (recData != NetworkEventType.Nothing)
        {
            switch (recData)
            {
                ...
                case NetworkEventType.DataEvent:
                    ...
                    {
                        names.Add(connectionId, message);
                        break;
                    }
                    else
                    {
                        SendMessageToAll($"{names[connectionId]}: {message}");
                        Debug.Log($"{names[connectionId]}: {message}");
                        break;
                    }

                case NetworkEventType.DisconnectEvent:
                    connectionIDs.Remove(connectionId);
                    SendMessageToAll($"{names[connectionId]} has disconnected.");
                    Debug.Log($"{names[connectionId]} has disconnected.");
